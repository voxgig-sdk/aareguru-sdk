...
class V2018Entity
  ...
  def make
    opts = @_entopts.dup
    V2018Entity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AareguruHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [V2018, Hash] the current V2018 data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AareguruHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of V2018 fields)
  def match_get
  ...
  # Load a single V2018.
  #
  # @param reqmatch [V2018LoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.V2018.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [V2018, Hash] the loaded V2018; raises AareguruError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AareguruHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
