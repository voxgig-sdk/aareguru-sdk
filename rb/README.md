...
The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Legacy` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = AareguruSDK.test(testopts, sdkopts)
```
...
raise a `AareguruError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Aareguru_sdk.rb       -- Main SDK module
...
The main module (`Aareguru_sdk`) exports the SDK class
