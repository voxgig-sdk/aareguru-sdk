...
`client.Legacy()` — each with a small set of operations (`load`)
...
```ts
try {
  const stuff = await client.Stuff().load({ service: "example" })
  console.log(stuff)
} catch (err) {
  console.error('load failed:', err)
}
...
const stuff = await client.Stuff().load({ service: 'example_service' })
...
await entity.load({ service: 'example_service' })
...
| `Stuff(data?)` | `StuffEntity` | Create a Stuff entity instance. |
| `V2018(data?)` | `V2018Entity` | Create a V2018 entity instance. |
| `tester(testopts?, sdkopts?)` | `AareguruSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `AareguruSDK.test(testopts?, sdkopts?)` | `AareguruSDK` | Create a test-mode client. |
...
```ts
const stuff = await client.Stuff().load({ service: 'service' })
```
...
```ts
const v2018 = await client.V2018().load({ city: 'city', end: 'end', start: 'start' })
```
...
await stuff.load({ service: "example" })
