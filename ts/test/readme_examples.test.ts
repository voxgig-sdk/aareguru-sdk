...
//     (\`new AareguruSDK(...)\` / \`AareguruSDK.test(...)\` become
//     \`AareguruSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"legacy":{"test01":{"id":"test01"}},"stuff":{"test01":{"id":"test01"}},"v2018":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AareguruSDK(...)\` and any
// \`AareguruSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AareguruSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AareguruSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
