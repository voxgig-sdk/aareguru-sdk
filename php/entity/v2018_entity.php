...
class V2018Entity
{
    ...
    {
        $opts = $this->_entopts;
        return new V2018Entity($this->_client, $opts);
    }

    /**
     * @param V2018|array $args V2018 data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AareguruHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return V2018|array The current V2018 data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of V2018 fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AareguruHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of V2018 fields).
     */
     ...
     * Load a single V2018.
     *
     * @param V2018LoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed V2018LoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return V2018|array The loaded V2018 as an assoc-array at the
     *   SDK boundary; throws AareguruError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = AareguruHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
