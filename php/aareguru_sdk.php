...
class AareguruSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = AareguruHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = AareguruHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!AareguruFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, AareguruFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return AareguruUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = AareguruHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = AareguruHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = AareguruHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new AareguruError($op . "_allow",
                "AareguruSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = AareguruHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new AareguruError("graphql_error",
                "AareguruSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Stuff($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_stuff === null) {
                $this->_stuff = new StuffEntity($this, null);
            }
            return $this->_stuff;
        }
        return new StuffEntity($this, $data);
    }
    ...
    public function V2018($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_v2018 === null) {
                $this->_v2018 = new V2018Entity($this, null);
            }
            return $this->_v2018;
        }
        return new V2018Entity($this, $data);
    }
