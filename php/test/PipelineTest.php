...
{
    private static function utility(): AareguruUtility
    {
        return new AareguruUtility();
    }
    ...
    private static function ctx(array $over = []): AareguruContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AareguruContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AareguruOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AareguruError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AareguruOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AareguruMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AareguruOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AareguruMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AareguruOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AareguruMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AareguruSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AareguruMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AareguruResponse([]),
            'result' => new AareguruResult([]),
        ]);
        [, $err] = AareguruMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AareguruSpec([]),
            'response' => null,
            'result' => new AareguruResult([]),
        ]);
        [, $err] = AareguruMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AareguruSpec(['step' => 's']),
            'response' => new AareguruResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AareguruResult([]),
        ]);
        [, $err] = AareguruMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AareguruSpec(['step' => 's']),
            'response' => new AareguruResponse(self::resp(200, ['v' => 1])),
            'result' => new AareguruResult([]),
        ]);
        [, $err] = AareguruMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AareguruSpec(['step' => 's']),
            'response' => new AareguruResponse(self::resp(200, ['v' => 2])),
            'result' => new AareguruResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AareguruMakeResponse::call($ctx);
    ...
    {
        $preset = new AareguruResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AareguruSpec([]),
            'response' => new AareguruResponse([]),
            'result' => new AareguruResult([]),
        ...
        [$got, $err] = AareguruMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AareguruResult([])]);
        [, $err] = AareguruMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AareguruSpec([]), 'result' => null]);
        [, $err] = AareguruMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AareguruSpec(['step' => 's']),
            'result' => new AareguruResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AareguruOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AareguruMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AareguruSpec(['step' => 's']),
            'result' => new AareguruResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AareguruOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AareguruMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AareguruResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AareguruSpec([]),
            'result' => new AareguruResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AareguruMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AareguruMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AareguruError('boom', 'boom');
        $utility->fetcher = function (AareguruContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AareguruSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AareguruMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AareguruContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AareguruMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AareguruResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AareguruContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AareguruSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AareguruMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AareguruResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AareguruSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AareguruMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AareguruMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AareguruMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AareguruResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AareguruDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AareguruResult(['ok' => false])]);
        $this->expectException(AareguruError::class);
        AareguruDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AareguruResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AareguruMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AareguruMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AareguruBaseFeature();
        $b = new AareguruBaseFeature();
        AareguruFeatureAdd::call($ctx, $a);
        AareguruFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AareguruBaseFeature
    {
        $f = new AareguruBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AareguruFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?AareguruSpec $spec): AareguruContext
    {
    ...
    private static function auth_bags(): AareguruSpec
    {
        return new AareguruSpec(['headers' => [], 'query' => []]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AareguruPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AareguruResponse(['headers' => null]),
            'result' => new AareguruResult([]),
        ]);
        AareguruResultHeaders::call($ctx);
