...
Dienschte hinger aare.guru<p>Bei Requests auf die API bitte frei wählbare <code>app</code> und <code>version</code> mitgeben.

Learn more about Aare.guru API at [aareguru.existenz.ch](https://aareguru.existenz.ch).
...
// stuff is the Stuff entity, populated with mock data
...
```go
client := sdk.Test()
result, err := client.Stuff(nil).Load(
    nil, nil,
)
```
...
| --- | --- | --- |
| **Legacy** | The Legacy entity (load). | `/current` |
| **Stuff** | The Stuff entity (load). | `/logs` |
| **V2018** | The V2018 entity (load). | `/v2018/history` |
...
// Load a specific legacy (returns the ENTITY; call data_get() for the record; throws on error)
$legacy = $client->Legacy()->load();
...
# Load a specific legacy (returns the ENTITY; call data_get for the record)
legacy = client.Legacy.load()
puts legacy
```
...
- Upstream API: [https://aare.guru/aare-info](https://aare.guru/aare-info)
