...
type Args struct {
	Entity string         `json:"entity" jsonschema:"legacy | stuff | v2018"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.AareguruSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "aareguru_list",
		Description: "List records from Aareguru. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "aareguru_load",
		Description: "Load a single record from Aareguru. " +
...
func runOp(client *sdk.AareguruSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.AareguruSDK, name string) (sdk.AareguruEntity, error) {
	switch strings.ToLower(name) {
	case "legacy":
		return client.Legacy(nil), nil
	case "stuff":
		return client.Stuff(nil), nil
	case "v2018":
		return client.V2018(nil), nil
