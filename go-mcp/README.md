...
MCP server exposing the Aareguru SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `aareguru_list` | `entity`, optional `query` map | First page of records as JSON |
| `aareguru_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `aareguru_list` and
`aareguru_load` tools become available in new sessions.
