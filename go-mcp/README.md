...
[MCP](https://modelcontextprotocol.io) server exposing the Aareguru SDK as
two agent tools — `aareguru_list` and `aareguru_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/aareguru-mcp)
make build

# 2. Provide credentials via the environment
export AAREGURU_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user aareguru \
  -- /absolute/path/to/aareguru-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./aareguru-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// aareguru_list: first page of records
{ "entity": "legacy" }
{ "entity": "legacy", "query": { } }

// aareguru_load: one record by id
{ "entity": "legacy", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/aareguru-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user aareguru \
     -- "$PWD"/dist/*/aareguru-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `aareguru_list` and `aareguru_load` tools now appear
   in new sessions. Ask the agent to *"list legacy using aareguru"*
   and it calls `aareguru_list` with `{"entity":"legacy"}`.
...
```sh
export AAREGURU_APIKEY=sk_live_xxx            # API key
export AAREGURU_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `aareguru_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `aareguru_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AAREGURU_APIKEY` | API key sent with every request. |
| `AAREGURU_BASE` | Optional override of the API base URL. |
...
```sh
./aareguru-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"aareguru_load","arguments":{"entity":"legacy","query":{"id":1}}}}'
```
