...
import (
	...
	omni "github.com/voxgig-sdk/aareguru-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AareguruError); is {
				out := map[string]any{
					"name":    "AareguruError",
...
func omniCtx(arg any, client *sdk.AareguruSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AareguruSDK); is {
			client = live
