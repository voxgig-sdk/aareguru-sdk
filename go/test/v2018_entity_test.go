...
func TestV2018Entity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set AAREGURU_TEST_V2018_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		v2018Ref01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.v2018")))
...
func v2018BasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("AAREGURU_TEST_V2018_ENTID")
	...
	idmapResolved := core.ToMapAny(env["AAREGURU_TEST_V2018_ENTID"])
