...
local V2018Entity = {}
V2018Entity.__index = V2018Entity


function V2018Entity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V2018Entity)
  self._name = "v2018"
  self._client = client
...
function V2018Entity:get_name()
  return self._name
end


function V2018Entity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V2018Entity.new(self._client, opts)
end
...
function V2018Entity:mark_deleted()
  self._deleted = true
end


function V2018Entity:deleted()
  return true == self._deleted
end


function V2018Entity:data_set(args)
  if args ~= nil then
...
function V2018Entity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V2018Entity:match_set(args)
  if args ~= nil then
...
function V2018Entity:match_get()
...
function V2018Entity:stream(action, args, callopts)
...
---@param reqmatch V2018LoadMatch
---@param ctrl? table
---@return V2018
---@return string? err
function V2018Entity:load(reqmatch, ctrl)
  local utility = self._utility
...
function V2018Entity:_run_op(ctx, post_done)
  local utility = self._utility
