-- Aareguru SDK Legacy entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local LegacyEntity = {}
LegacyEntity.__index = LegacyEntity


function LegacyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LegacyEntity)
  self._name = "legacy"
  self._client = client
...
function LegacyEntity:get_name()
  return self._name
end


function LegacyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LegacyEntity.new(self._client, opts)
end
...
function LegacyEntity:mark_deleted()
  self._deleted = true
end


function LegacyEntity:deleted()
  return true == self._deleted
end


function LegacyEntity:data_set(args)
  if args ~= nil then
...
function LegacyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LegacyEntity:match_set(args)
  if args ~= nil then
...
function LegacyEntity:match_get()
...
function LegacyEntity:stream(action, args, callopts)
...
---@param reqmatch LegacyLoadMatch
---@param ctrl? table
---@return Legacy
---@return string? err
function LegacyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function LegacyEntity:_run_op(ctx, post_done)
  local utility = self._utility
