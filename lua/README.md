...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Legacy()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local stuff, err = client:Stuff():load({ service = "example" })
if err then error(err) end
```
...
local result, err = client:Stuff():load({ service = "example" })
...
```lua
local stuff, err = client:Stuff():load({ service = "service" })
```
...
```
lua/
├── aareguru_sdk.lua    -- Main SDK module
...
The main module (`aareguru_sdk`) exports the SDK constructor
