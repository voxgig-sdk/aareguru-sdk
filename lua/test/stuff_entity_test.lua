...
describe("StuffEntity", function()
  ...
  it("should run basic flow", function()
    ...
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "stuff." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set AAREGURU_TEST_STUFF_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    assert.is_not_nil(stuff_ref01_data_dt0_loaded)
...
function stuff_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("AAREGURU_TEST_STUFF_ENTID")
