...
//
// NewAareguruSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.AareguruSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewAareguruSDK(opts)
}
...
func fcCandidates(client *sdk.AareguruSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.AareguruSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.AareguruSDK, name string) any {
