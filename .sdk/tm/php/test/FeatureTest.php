...
class FtCtrl extends AareguruControl
{
...
class FtClient
{
    ...
    private AareguruUtility $utility;

    public function __construct(AareguruUtility $utility, array $options)
    {
    ...
    public function get_utility(): AareguruUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AareguruContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AareguruContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AareguruContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AareguruConfig::make_feature($name);
    ...
    private static function buildUrl(AareguruSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AareguruError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AareguruResponse($fetched);
        }
    ...
    private function fail(AareguruContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AareguruContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
